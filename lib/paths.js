...
/**
 * Keeping every file inkd touches inside the folder it was opened on.
 *
 * inkd writes files, and people open folders they did not write -- a
...
/** A request for something outside the root, or somewhere inkd will not go. */
...
function segmentsOf(rel) {
  ...
  if (rel.startsWith('/')) throw new ForbiddenPathError('Paths are relative to the folder inkd opened.');
  ...
  for (const segment of segments) {
    ...
    if (PROTECTED_DIRS.has(segment)) {
      throw new ForbiddenPathError(`inkd does not open anything inside ${segment}/.`);
    }
  }
  return segments;
}
